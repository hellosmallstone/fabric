...
func TestUpdateOrdererMSP(t *testing.T) {
	...
	channelGroup, err := baseOrdererChannelGroup(t, ConsensusTypeSolo)
	gt.Expect(err).NotTo(HaveOccurred())
	...
	err = c.UpdateOrdererMSP(ordererMSP, "OrdererOrg")
	gt.Expect(err).NotTo(HaveOccurred())
...
func TestUpdateConsortiumMspFailure(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.spec, func(t *testing.T) {
			...
			err = c.UpdateConsortiumMsp(consortiumOrg1MSP, tc.consortiumName, tc.orgName)
...
func TestUpdateOrdererMSPFailure(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.spec, func(t *testing.T) {
			...
			err = c.UpdateOrdererMSP(ordererMSP, tc.orgName)
			gt.Expect(err).To(MatchError(tc.expectedErr))
		})
	}
}
...
func TestUpdateConsortiumMsp(t *testing.T) {
	...
	err = c.UpdateConsortiumMsp(consortiumOrg1MSP, "Consortium1", "Org1")
	gt.Expect(err).NotTo(HaveOccurred())
